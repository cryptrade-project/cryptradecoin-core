...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::CRCO, nValue);
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (cryptradecoin.conf).");
            QMessageBox::warning(this, "CryptradeCoin Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
{
    QString CSS = "body {font-size:12px; color:#e0e0e0; background-color:#191a20;}\n a, span { font-family: monospace; }\n span.addr {color:#0e9bc3; font-weight: bold;}\n table tr td {padding: 3px; border: 1px solid black; background-color: #2a2b34;}\n td.d0 {font-weight: bold; color:#e0e0e0;}\n h2, h3 { white-space:nowrap; color:#e0e0e0;}\n a { color:#00a9e9; text-decoration:none; }\n a.nav {color:#e0e0e0;}\n";
