...
#include "ui_zcrcocontroldialog.h"
...
std::list<std::string> ZCrcoControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZCrcoControlDialog::listMints;

ZCrcoControlDialog::ZCrcoControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZCrcoControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZCrcoControlDialog::~ZCrcoControlDialog()
{
    delete ui;
}

void ZCrcoControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCrcoControlDialog::updateList()
{
...
void ZCrcoControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCrcoControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZCrco_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCrcoControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZCrcoControlDialog::GetSelectedMints()
{
...
void ZCrcoControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
