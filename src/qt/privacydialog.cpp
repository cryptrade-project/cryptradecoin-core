                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zCRCO ought to be enough for anybody." - Bill Gates, 2017
    ui->zCRCOpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCRCOSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzCRCO->setEnabled(false);
        ui->pushButtonMintzCRCO->setToolTip(tr("zCRCO is currently disabled due to maintenance."));

        ui->pushButtonSpendzCRCO->setEnabled(false);
        ui->pushButtonSpendzCRCO->setToolTip(tr("zCRCO is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCRCOpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCRCO_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCRCO is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCRCO...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCRCO in ") +
...
void PrivacyDialog::on_pushButtonSpendzCRCO_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCRCO is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zCRCO
        sendzCRCO();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCRCO
    sendzCRCO();
}

void PrivacyDialog::on_pushButtonZCrcoControl_clicked()
{
    ZCrcoControlDialog* zCRCOControl = new ZCrcoControlDialog(this);
    zCRCOControl->setModel(walletModel);
    zCRCOControl->exec();
}

void PrivacyDialog::setZCrcoControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCRCOSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCRCO()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid CryptradeCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCRCOpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCRCO is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CRCO </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCRCO</b>";
    ...
    // use mints from zCRCO selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCrcoControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZCrcoControlDialog::GetSelectedMints();
    }
    ...
    // Clear zcrco selector in case it was used
    ZCrcoControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zCRCO Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " CryptradeCoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zCRCO Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCRCO </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCRCO "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCRCO "));
    ui->labelzCRCOAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzCRCOSyncStatus->setVisible(fShow);
}
