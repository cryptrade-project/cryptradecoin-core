...
#if defined(NDEBUG)
#error "CryptradeCoin cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 1 * 60 * 60; // 1hour
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zCRCO spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zCRCO spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zCRCO spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nPrevHeight)
{
    ...
    if (nPrevHeight >= 0 && nPrevHeight < Params().LAST_POW_BLOCK()) {
        nSubsidy = 4000 * COIN; // Premined coins, 240,000, 1%
    } else if (nPrevHeight >= Params().LAST_POW_BLOCK() && nPrevHeight < 4320) {
        nSubsidy = 0.1 * COIN;
    } else if (nPrevHeight >= 4320 && nPrevHeight < 15000) {
        nSubsidy = 2 * COIN;
    } else if (nPrevHeight >= 15000 && nPrevHeight < 30000) {
        nSubsidy = 4 * COIN;
    } else if (nPrevHeight >= 30000 && nPrevHeight < 45000) {
        nSubsidy = 6 * COIN;
    } else if (nPrevHeight >= 45000 && nPrevHeight < 60000) {
        nSubsidy = 8 * COIN;
    } else if (nPrevHeight >= 60000 && nPrevHeight < 75000) {
        nSubsidy = 10 * COIN;
    } else if (nPrevHeight >= 75000 && nPrevHeight < 90000) {
        nSubsidy = 12 * COIN;
    } else if (nPrevHeight >= 90000 && nPrevHeight < 105000) {
        nSubsidy = 14 * COIN;
    } else if (nPrevHeight >= 105000 && nPrevHeight < 120000) {
        nSubsidy = 16 * COIN;
    } else if (nPrevHeight >= 120000 && nPrevHeight < 135000) {
        nSubsidy = 14 * COIN;
    } else if (nPrevHeight >= 135000 && nPrevHeight < 150000) {
        nSubsidy = 12 * COIN;
    } else if (nPrevHeight >= 150000 && nPrevHeight < 165000) {
        nSubsidy = 10 * COIN;
    } else if (nPrevHeight >= 165000 && nPrevHeight < 180000) {
        nSubsidy = 8 * COIN;
    } else if (nPrevHeight >= 180000 && nPrevHeight < 195000) {
        nSubsidy = 6 * COIN;
    } else {
        nSubsidy = 4 * COIN;
    }

    // yearly decline of production by 10% per year until reached max coin ~24M.
    for (int i = Params().SubsidyHalvingInterval(); i <= nPrevHeight; i += Params().SubsidyHalvingInterval()) {
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	// 90% for Masternodes since the last pow block
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from CryptradeCoin
...
{
    RenameThread("cryptradecoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZCRCOMinted()
{
...
void RecalculateZCRCOSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zCRCO supply
...
bool RecalculateCRCOSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zCRCO with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zCRCOSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert CRCO to zCRCO
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zCRCO spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zCRCO serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zCRCO mints and %d zCRCO spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // CryptradeCoin: We use certain sporks during IBD, so check to see if they are
